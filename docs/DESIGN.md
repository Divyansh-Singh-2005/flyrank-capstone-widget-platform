...
- Delivery: the `/widget.js` loader (5 min cache, ETag) injects `/static/widget.v{WIDGET_BUNDLE_VERSION}.js` (1 year, immutable). The bundle renders inside Shadow DOM using only `textContent` (no `innerHTML`), adds the hidden honeypot, and reuses one Idempotency-Key per attempt until it succeeds.
- The config endpoint returns only public fields plus `honeypot_field` and `submit_url`. Its strong ETag is a SHA-256 of the body, and `If-None-Match` gets a 304.
- Stats: summary (totals, last 24 h / 7 d, per widget), timeseries (day/hour buckets in UTC, zero-filled), and geo breakdown (`Unknown` = not enriched). All queries filter by `tenant_id`. Bucket units are SQL literals, because a bound parameter in `date_trunc` breaks GROUP BY under psycopg3 server-side binding.
- `/dashboard`: a same-origin static page that keeps the token in memory and renders with `textContent`.
- Docker: the one-shot `migrate` service runs Alembic before `api` and `worker` start. The worker turns SIGTERM into a clean stop, and `demo` serves the customer site on :5500. Compose has fallback defaults, so it starts without `.env`.
- Tests run against a separate `<db>_test` database that is created and migrated once per session.
- `EVIDENCE.md` is generated by `scripts/collect_evidence.py` from a live run instead of being written by hand.